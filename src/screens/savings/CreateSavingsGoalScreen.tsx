...
const getGoalEmoji = (name: string): string => {
  ...
  if (l.includes("trip") || l.includes("travel") || l.includes("flight"))
    return "✈️";
  if (l.includes("car") || l.includes("vehicle")) return "🚗";
  if (l.includes("house") || l.includes("home") || l.includes("rent"))
    return "🏠";
  if (l.includes("emergency") || l.includes("fund")) return "🛡️";
  if (l.includes("wedding")) return "💍";
  if (l.includes("school") || l.includes("edu") || l.includes("tuition"))
    return "🎓";
...
export const CreateSavingsGoalScreen = () => {
  ...
  const editingGoal = goalParam
    ? (JSON.parse(goalParam as string) as SavingsGoal)
    : null;
  const isEditing = !!editingGoal;

  const [name, setName] = useState(editingGoal?.name || "");
  const [targetAmount, setTargetAmount] = useState(
    editingGoal?.targetAmount ? editingGoal.targetAmount.toLocaleString() : "",
  );
    ...
    <View style={[styles.container, { paddingTop: insets.top }]}>
      ...
      >
        ...
        >
          ...
          <InputField
            ...
            onChangeText={(t) =>
              setTargetAmount(formatAmount(t.replace("₦", "")))
            }
          ...
          {!isEditing && (
            <InputField
              ...
              onChangeText={(t) =>
                setInitialDeposit(formatAmount(t.replace("₦", "")))
              }
              keyboardType="numeric"
      ...
      <View style={[styles.footer, { paddingBottom: insets.bottom + 12 }]}>
        <TouchableOpacity
          style={[
            styles.createBtn,
            isFormValid ? styles.createBtnActive : styles.createBtnDisabled,
          ]}
          onPress={handleSave}
